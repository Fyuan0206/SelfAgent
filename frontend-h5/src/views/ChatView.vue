...
import { sendTextMessage, sendMultimodalMessage, compressHistory } from '@/api/chat'
...
const sendMessage = async () => {
  ...
  try {
    ...
    if (chatStore.messages.length > 12) {
      const keep = 2
      const older = chatStore.messages.slice(0, chatStore.messages.length - keep).map(m => ({
        role: m.role,
        content: m.content,
        timestamp: m.timestamp.toISOString()
      }))
      try {
        const summaryResp = await compressHistory(older)
        chatStore.messages = [
          {
          ...
          ...chatStore.messages.slice(chatStore.messages.length - keep)
        ]
      } catch (e) {
      }
    }
  ...
  <div class="flex flex-col h-screen pb-14">
    ...
    >
      <div
        ...
        :key="message.id"
        class="message-enter"
        :class="message.role === 'user' ? 'flex justify-end' : (message.role === 'assistant' ? 'flex justify-start' : 'flex justify-center')"
      >
        <!-- 系统摘要消息 -->
        <div v-if="message.role === 'system'" class="max-w-[85%]">
          <div class="bg-indigo-50 dark:bg-slate-700 rounded-2xl px-4 py-3 shadow-sm text-center">
            <p class="text-indigo-700 dark:text-indigo-300 text-xs font-medium">会话摘要</p>
            <p class="text-gray-700 dark:text-gray-200 text-sm whitespace-pre-wrap mt-1">{{ message.content }}</p>
          </div>
