...
class DBTSkillsTool(BaseToolkit):
    ...
    async def _ensure_initialized(self):
        ...
        try:
            ...
            # 初始化基础数据（演示用）
            # 注意：init_database 使用的是 AsyncSessionLocal，它默认连接到 ./data/dbt_skills.db
            # 这里我们想在内存数据库中初始化，所以需要一点技巧
            
            # 导入数据
            from app.modules.dbt.db.init_data import DBT_MODULES, DBT_SKILLS, MATCHING_RULES
            from app.modules.dbt.models.database import DBTModule, DBTSkill, SkillMatchingRule
