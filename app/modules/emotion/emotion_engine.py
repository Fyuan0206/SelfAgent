...
# 导入配置加载器
from .config_loader import EmotionConfigLoader

# 导入本地模块
from .emotion_extractor import EmotionExtractor, EmotionFeatures
from app.services.routing import IntelligentRouter, RouteLevel
from app.services.intervention import RiskAssessmentEngine
from app.services.profile import (
    EmotionProfileManager, EmotionSnapshot,
    AdvancedEmotionProfileManager, AdvancedEmotionProfile
)
...
class EmotionRecognitionEngine:
    ...
    def __init__(self, config: Union[Dict, str, None] = None, use_advanced_profile: bool = True):
        # 加载配置
        if config is None or isinstance(config, str):
            # 从.env文件加载配置
            config_loader = EmotionConfigLoader(config)
            self.config = config_loader.get_config()
        elif isinstance(config, dict):
            # 直接使用传入的配置
            self.config = config
        else:
            raise ValueError("config参数必须是Dict、str（.env路径）或None")
    ...
    def analyze(self, text: str,
        ...
        if self.use_advanced_profile:
            # 高级快照 - 包含更多元数据
            # 导入 AdvancedEmotionSnapshot (延迟导入以避免循环依赖)
            from app.services.profile.advanced_emotion_profile import EmotionSnapshot as AdvancedEmotionSnapshot
            ...
            # 基础快照
            # 导入 EmotionSnapshot
            from app.services.profile.emotion_profile import EmotionSnapshot
